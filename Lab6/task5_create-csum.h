//
// Created by sapiest on 13.05.2020.
//

#ifndef TESSOS_TASK5_CREATE_CSUM_H
#define TESSOS_TASK5_CREATE_CSUM_H
#include <bits/types/FILE.h>
#include <stdio.h>
#include <stdlib.h>
#define BUFF_SIZE 4096

static const unsigned char crc8Table[256] = {
    };


static unsigned char crc8(const unsigned char* buf, unsigned int buf_size){
  unsigned char sum = 0xFF;
  for (int i = 0; i < buf_size; ++i) {
    sum = crc8Table[sum ^ buf[i]];
  }
  return sum;
}

static int create_csum(const char* file_name, const char* out_file_name){
  FILE *in, *out;
  unsigned char buff[BUFF_SIZE];
  unsigned int size;

  if(!(in = fopen(file_name, "r"))){
    fprintf(stderr, "File's not open\n");

    return EXIT_FAILURE;
  }

  if(!(out = fopen(out_file_name, "w"))){
    fprintf(stderr, "File's not open\n");

    return EXIT_FAILURE;
  }

  while ((size = fread(&buff, 1, BUFF_SIZE, in)) != 0){
    fprintf(out, "%c", crc8(buff, size));
  }

  fclose(out);
  fclose(in);

  return EXIT_SUCCESS;
}


#endif // TESSOS_TASK5_CREATE_CSUM_H
